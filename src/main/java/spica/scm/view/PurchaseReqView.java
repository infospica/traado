 ...
 */
@Named(value = "purchaseReqView")
@ViewScoped
public class PurchaseReqView implements Serializable {
   ...
   * @param purchaseReqItem
   */
  //FIXME serach for similar methods and pass main
  public void productSelectEvent(MainView main, PurchaseReqItem purchaseReqItem) {
    try {
      if (purchaseReqItem.getProductId() != null) {
        purchaseReqItem.setProductName(purchaseReqItem.getProductId().getProductName());
  ...
  private Integer getProdcutTotalQuantity() {
    Integer qty = 0;
    for (PurchaseReqItem pq : getPurchaseReqItemList()) {
      qty += pq.getQtyRequired() != null ? pq.getQtyRequired() : 0;
    }
    return qty;
  }
