		...
		$(ob).mcautocomplete({
			...
			change: function (event, ui) {
				var r = dataTable.findPos(this),
					arg = $.escapeSelector($(this).attr('data-update'));
				r = r[1];
				if ($(this).val() !== $.escapeSelector($(this).attr('data-value'))) {
					...
					switch (arg) {
						...
						case 'danger':
							...
							popup.open('#' + $.escapeSelector($(this).attr('data-dialog')), 768, 600, {
								units: 'px',
								xsource: $(this).attr('data-url'),
								setText: 'Create Product',
								setDataFocus: $.escapeSelector($(this).data('focus'))
							});
							break;
